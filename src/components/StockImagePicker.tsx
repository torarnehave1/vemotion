  ...
  return (
    <div className="space-y-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-100/40 dark:bg-slate-800/40 p-3">
      <div className="flex items-center justify-between gap-2">
        <label className="text-xs text-slate-500 dark:text-slate-400">Or search stock photos</label>
          ...
          {(['unsplash', 'pexels'] as StockProvider[]).map((p) => (
            <button
              ...
              className={`px-2 py-1 text-[11px] capitalize transition ${
                provider === p ? 'bg-sky-600 text-slate-900 dark:text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              {p}
            </button>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-slate-900 dark:text-white rounded-lg text-xs font-medium transition"
        >
          ...
          {results.map((image, i) => (
            <button
              ...
              className={`relative aspect-square rounded overflow-hidden border-2 transition disabled:opacity-50 ${
                pickedUrl === image.url ? 'border-sky-400 ring-2 ring-sky-400/40' : 'border-slate-200 dark:border-slate-700 hover:border-slate-400'
              }`}
            >
