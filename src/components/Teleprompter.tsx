...
export const Teleprompter: React.FC<Props> = ({ lines, currentTime, onSeek, onClose }) => {
  ...
  const navBtn = (disabled: boolean) =>
    `p-1 rounded ${disabled ? 'text-slate-600 cursor-default' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800'}`;

  return createPortal(
    <div style={{ position: 'fixed', left: pos.x, top: pos.y, zIndex: 1000, width: W, maxWidth: '96vw' }}>
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl" style={{ background: 'rgba(7,11,20,0.97)' }}>
        <div
          onPointerDown={onHeaderDown}
          className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-slate-800 cursor-grab active:cursor-grabbing select-none"
        >
          ...
          <div className="flex items-center gap-1">
            ...
            <button onPointerDown={stop} onClick={() => setFontSize((s) => Math.max(20, s - 4))} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1" aria-label="Smaller text"><Minus className="w-4 h-4" /></button>
            <span className="text-[11px] text-slate-500 w-7 text-center select-none">{fontSize}</span>
            <button onPointerDown={stop} onClick={() => setFontSize((s) => Math.min(110, s + 4))} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1" aria-label="Larger text"><Plus className="w-4 h-4" /></button>
            <button onPointerDown={stop} onClick={onClose} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1 ml-1" aria-label="Close teleprompter"><X className="w-4 h-4" /></button>
          </div>
        </div>
        <div className="px-6 py-5">
          <p className="text-slate-900 dark:text-white" style={{ fontSize, lineHeight: 1.3, fontWeight: 600, textAlign: 'center' }}>
            {cur ? cur.text : '— no narration (add via Script) —'}
          </p>
          {nxt && (
            <p className="mt-3 text-slate-500 dark:text-slate-400" style={{ fontSize: Math.round(fontSize * 0.5), textAlign: 'center' }}>
              next: {nxt.text}
            </p>
          )}
        </div>
