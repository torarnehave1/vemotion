...
export default function TrainingVideosModal({ onClose }: TrainingVideosModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <div>
            <h2 className="text-lg font-semibold text-white">Training Videos</h2>
          ...
          <div className="flex items-center gap-2">
            <button onClick={load} className="text-slate-400 hover:text-white transition" title="Refresh">
        ...
        <div className="p-6 overflow-y-auto space-y-3">
          ...
          {!loading && shown.length === 0 && (
            <p className="text-slate-400 text-sm py-8 text-center">No training videos{filter !== 'all' ? ` (${filter})` : ''}.</p>
          )}
          ...
          {shown.map((v) => {
            ...
            return (
              <div key={v.key} className="rounded-xl border border-slate-700 bg-slate-800/60 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      ...
                      disabled={busy}
                      className="text-slate-300 hover:text-white p-1.5 disabled:opacity-50"
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                    <label className="text-xs text-slate-400">
                      ...
                      <select
                        ...
                        className="mt-1 w-full bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-200"
                      >
                    ...
                    {d.audienceMode === 'emails' && (
                      <label className="text-xs text-slate-400 md:col-span-3">
                        ...
                        <textarea
                          ...
                          className="mt-1 w-full bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-200 font-mono"
                        />
                      </label>
                    )}
                    <div className="md:col-span-3 flex justify-end">
                      <button
                        ...
                        disabled={busy}
                        className="bg-sky-600 hover:bg-sky-500 disabled:bg-slate-700 text-white text-sm font-medium rounded-lg px-4 py-1.5 transition flex items-center gap-2"
                      >
